...
func TestAuditorCannotUpdateArea(t *testing.T) {
	configuration := config.Config{
		DBDriver:    "sqlite",
		DBDSN:       "file:auditor-area-update-test?mode=memory&cache=shared",
		JWTSecret:   "auditor-area-update-test-secret-with-more-than-24-characters",
		AutoMigrate: true,
		SeedData:    true,
	}
	...
	body := `{"name":"审计员尝试修改","boundary_geojson":{"type":"Feature","properties":{},"geometry":{"type":"Polygon","coordinates":[[[0,0],[100,0],[100,100],[0,100],[0,0]]]}},"target_resolution_m":20,"coordinate_system":"EPSG:32650","default_swath_m":180,"owner_team":"测试组","status":"active","expected_version":1}`
	request := httptest.NewRequest(http.MethodPut, "/api/v1/areas/1", strings.NewReader(body))
